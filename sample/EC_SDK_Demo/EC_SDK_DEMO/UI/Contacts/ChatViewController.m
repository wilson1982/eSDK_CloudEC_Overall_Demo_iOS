...
- (IBAction)longPressGesture:(UILongPressGestureRecognizer *)sender
{
    ...
    switch (sender.state) {
        ...
        case UIGestureRecognizerStateCancelled:
        {
            ...
            BOOL success = [[ESpaceMediaService sharedInstance] stopRecord:&duration];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([mediaType isEqualToString:@"public.movie"]) {
        ...
        DDLogInfo(@"movie");
        isImage = NO;
        data = [NSData dataWithContentsOfURL:sourceURL];
        umResource = [[ESpaceUMVideoResource alloc] init];
        umResourceURL = [NSURL exportURLInCacheDirecotry:@"MP4" error:&error];
        umResource.umType = ESpaceUMTypeVideo;
        AVURLAsset *urlAsset = [AVURLAsset assetWithURL:sourceURL];
        float second = 0;
        second = urlAsset.duration.value/urlAsset.duration.timescale;
        umResource.duration = second;
    }
