...
- (void)cameraChangeBtnClicked:(id)sender {
    if (_isCameraOpen) {
        if ([self isUseTupVideo]) {
            ...
            BOOL isSuccess = [[ManagerService callService] switchCameraIndex:_cameraCaptureIndex callId:_currentTupCallInfo.stateInfo.callId];
            if (isSuccess) {
...
- (void)cameraHandleBtnClicked:(id)sender {
    if ([self isUsmDataConf]) {
        ...
        if ([self isVideoConf]) {
            [[ManagerService callService] switchCameraOpen:!_isCameraOpen callId:_currentTupCallInfo.stateInfo.callId];
