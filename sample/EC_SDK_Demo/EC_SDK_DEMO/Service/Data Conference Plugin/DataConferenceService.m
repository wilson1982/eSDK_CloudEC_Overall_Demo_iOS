...
-(BOOL)createDataConfObjectWithParams:(DataConfParam *)dataConfParams
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        /*
         * Need report param int value ,avoid overflow
         */
        if (shortSipNum.length > 7) {
            shortSipNum = [shortSipNum substringFromIndex:shortSipNum.length-7];
        }
        
        int userID = [shortSipNum intValue];
        ...
        strncpy(confInfo->user_log_uri, [shortSipNum UTF8String],shortSipNum.length);
        strncpy(confInfo->user_name, [loginInfo.account UTF8String],loginInfo.account.length);
...
- (void)dealDataConfUserEnterEventWithNumber:(NSString *)userNumber
    ...
    if (![userName isEqualToString:mine.account])
    {
        if (userid != 0)
        {
            _selfUserId = userid;
    ...
    for (ConfAttendeeInConf *attendee in joinedAttenees) {
        if ([attendee.number isEqualToString:userNumber]) {
