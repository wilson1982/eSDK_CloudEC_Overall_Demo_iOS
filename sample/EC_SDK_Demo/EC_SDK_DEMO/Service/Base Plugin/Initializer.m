...
TUP_VOID onTupCallCallBack(TUP_UINT32 msgid, TUP_UINT32 param1, TUP_UINT32 param2, void *data)
{
    ...
    if (msgid == CALL_E_EVT_SIPACCOUNT_INFO || msgid == CALL_E_EVT_CALL_LOGOUT_NOTIFY || msgid == CALL_E_EVT_FORCEUNREG_INFO)
    {
...
/**
 * tup_ctd_register_process_notifiy的接口参数回调CTD_FN_CALLBACK_PTR
 */
void onTupCtdCallBack(TUP_UINT32 msgid, TUP_UINT32 param1, TUP_UINT32 param2, TUP_VOID *data)
{
    Notification *ctdNotify = [[Notification alloc] initWithMsgId:msgid param1:param1 param2:param2 data:data];
    [g_callDelegate callModule:CALL_CTD_MODULE notication:ctdNotify];
}
...
/**
 * tup_eaddr_init的接口参数回调TUP_EADDR_FN_CALLBACK_PTR
 */
TUP_VOID onTUPContactNotifications(TUP_UINT32 msgid,TUP_VOID *data) {
    ContactNotification *contactNotify = [[ContactNotification alloc] initWithContactMsgId:msgid data:data];
    [g_contactDelegate contactModule:CONTACT_MODULE notification:contactNotify];
}
...
+ (BOOL)startupWithLogPath:(NSString *)logPath
{
    ...
    [self configBFCP];
    [self configIPTBusiness];
    
    result = [self ctdStartupWithLogPath:logPath];
    if (!result) {
        return NO;
    }
    ...
    result = [self contactStartupWithLogPath:logPath];
    if (!result) {
        return NO;
    }
...
/**
 * This method is used to init contact service.
 * 初始化联系人模块
 *@param logPath NSString
 *@return BOOL
 */
+ (BOOL)contactStartupWithLogPath:(NSString *)logPath {
    TUP_EADDR_S_LOG_PARA logParams;
    memset(&logParams, 0, sizeof(TUP_EADDR_S_LOG_PARA));
    NSString *path = [logPath stringByAppendingString:@"/contact"];
    logParams.enLevel = TUP_EADDR_DEBUG;
    logParams.ulMaxFileSize = 10*1024;
    strcpy(logParams.acLogPath, [path UTF8String]);
    TUP_RESULT configLogResult = tup_eaddr_set_log(&logParams);
    DDLogInfo(@"tup_eaddr_set_log result: %d",configLogResult);
    TUP_RESULT result = tup_eaddr_init(&onTUPContactNotifications);
    DDLogInfo(@"tup_eaddr_init result: %d",result);
...
+ (void)configCallBusiness
{
    ...
    TUP_UINT32 envSolution = CALL_E_NET_UC2X;
    ...
    NSString *userAgentStr = USER_AGENT_UC;
...
/**
 * This method is used to init ctd service.
 * 初始化ctd模块
 *@param logPath NSString
 *@return BOOL
 */
+ (BOOL)ctdStartupWithLogPath:(NSString *)logPath
{
    // init ctd
    NSString *path = [logPath stringByAppendingString:@"/ctd"];
    tup_ctd_log_start(CTD_E_LOG_DEBUG, 1*1024, 1, (TUP_CHAR*)[path UTF8String]);
    TUP_BOOL enableReport = 1;
    TUP_RESULT initResult = tup_ctd_init(enableReport);
    DDLogInfo(@"tup_ctd_init result : %d",initResult);
    if (initResult != TUP_SUCCESS) {
        return NO;
    }
    
    TUP_RESULT result = tup_ctd_register_process_notifiy(onTupCtdCallBack);
    DDLogInfo(@"tup_ctd_register_process_notifiy: %d", result);
    return result == TUP_SUCCESS;
}
...
/**
 This method is used to config IPT business
 配置IPT业务
 */
+ (void)configIPTBusiness
{
    [self configDNDIPTServerWithActiveCode:@"*56*" deactiveCode:@"#56#"];
    [self configCallWaitIPTServerWithActiveCode:@"*43#" deactiveCode:@"#43#"];
    [self configCFBIPTServerWithActiveCode:@"**67*" deactiveCode:@"##67#"];
    [self configCFUIPTServerWithActiveCode:@"**21*" deactiveCode:@"##21#"];
    [self configCFNAIPTServerWithActiveCode:@"**61*" deactiveCode:@"##61#"];
    [self configCFNRIPTServerWithActiveCode:@"**45*" deactiveCode:@"##45#"];
}


/**
 * This method is used to config service authority:DND
 * 业务权限配置：DND
 *@param activeCode NSString
 *@param deactiveCode NSString
 */
+(void)configDNDIPTServerWithActiveCode:(NSString *)activeCode deactiveCode:(NSString *)deactiveCode
{
    TUP_RESULT configResult = TUP_FAIL;
...
/**
 * This method is used to config service authority:CALLWAIT
 * 业务权限配置：CALLWAIT
 *@param activeCode NSString
 *@param deactiveCode NSString
 */
+(void)configCallWaitIPTServerWithActiveCode:(NSString *)activeCode deactiveCode:(NSString *)deactiveCode
{
    TUP_RESULT configResult = TUP_FAIL;
    CALL_S_SERVICE_RIGHT_CFG serviceRightCfg;
    memset(&serviceRightCfg, 0, sizeof(serviceRightCfg));
...
/**
 * This method is used to config service authority:CFU
 * 业务权限配置：CFU
 *@param activeCode NSString
 *@param deactiveCode NSString
 */
+(void)configCFUIPTServerWithActiveCode:(NSString *)activeCode deactiveCode:(NSString *)deactiveCode
{
    TUP_RESULT configResult = TUP_FAIL;
    CALL_S_SERVICE_RIGHT_CFG serviceRightCfg;
    memset(&serviceRightCfg, 0, sizeof(serviceRightCfg));
...
/**
 * This method is used to config service authority:CFB
 * 业务权限配置：CFB
 *@param activeCode NSString
 *@param deactiveCode NSString
 */
+(void)configCFBIPTServerWithActiveCode:(NSString *)activeCode deactiveCode:(NSString *)deactiveCode
{
    TUP_RESULT configResult = TUP_FAIL;
    CALL_S_SERVICE_RIGHT_CFG serviceRightCfg;
    memset(&serviceRightCfg, 0, sizeof(serviceRightCfg));
...
/**
 * This method is used to config service authority:CFNA
 * 业务权限配置：CFNA
 *@param activeCode NSString
 *@param deactiveCode NSString
 */
+(void)configCFNAIPTServerWithActiveCode:(NSString *)activeCode deactiveCode:(NSString *)deactiveCode
{
    TUP_RESULT configResult = TUP_FAIL;
    CALL_S_SERVICE_RIGHT_CFG serviceRightCfg;
    memset(&serviceRightCfg, 0, sizeof(serviceRightCfg));
...
/**
 * This method is used to config service authority:CFNR
 * 业务权限配置：CFNR
 *@param activeCode NSString
 *@param deactiveCode NSString
 */
+(void)configCFNRIPTServerWithActiveCode:(NSString *)activeCode deactiveCode:(NSString *)deactiveCode
{
    TUP_RESULT configResult = TUP_FAIL;
    CALL_S_SERVICE_RIGHT_CFG serviceRightCfg;
    memset(&serviceRightCfg, 0, sizeof(serviceRightCfg));
