...
/**
 *This method is used to config bussiness account, ctd server, token and conf service when token update
 *token刷新时重新配置token相关的业务
 */
- (void)configOtherService
{
    LoginServerInfo *info = [self obtainAccessServerInfo];
    [[ManagerService callService] configBussinessAccount:info.sipAccount token:[self obtainToken]];
    [[ManagerService callService] ctdConfigServer:info.authServer port:info.authServerPort];
    [[ManagerService confService] configToken:[self obtainToken]];
    [ManagerService confService].uPortalConfType = [self configDeployMode:info.deployMode];
    [[ManagerService contactService] configUportalInfo:info token:[self obtainToken]];
}
